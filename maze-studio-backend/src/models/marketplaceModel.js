...
async function listPublished({ search = "", type = "" }) {
  const result = await pool.query(`
    SELECT offering.id, offering.title, offering.description, offering.offering_type,
      ...
      COUNT(cohort.id) FILTER (WHERE cohort.status IN ('OPEN','ACTIVE'))::int open_cohort_count
    ...
    WHERE offering.status='PUBLISHED' AND journey.status='PUBLISHED'
      ...
      AND ($1::text='' OR offering.title ILIKE '%'||$1::text||'%' OR journey.title ILIKE '%'||$1::text||'%' OR CONCAT_WS(' ',educator.first_name,educator.last_name) ILIKE '%'||$1::text||'%')
...
async function listOpenCohorts(offeringId) {
  return (await pool.query(`
    SELECT cohort.id,cohort.title,cohort.starts_at,cohort.ends_at,cohort.timezone,cohort.capacity,
      COUNT(member.id) FILTER(WHERE member.status='ACTIVE')::int member_count
    FROM cohorts cohort LEFT JOIN cohort_members member ON member.cohort_id=cohort.id
    WHERE cohort.offering_id=$1::uuid AND cohort.status IN ('OPEN','ACTIVE')
    GROUP BY cohort.id HAVING COUNT(member.id) FILTER(WHERE member.status='ACTIVE') < cohort.capacity
    ORDER BY cohort.starts_at NULLS LAST`, [offeringId])).rows;
}
...
async function createOrder(offering, userId, cohortId, status = "PENDING", booking = {}) {
  const count=offering.offering_type==="ONE_TO_ONE"?Number(booking.sessionCount):null;
  const monthly=offering.offering_type==="ONE_TO_ONE"&&booking.frequency==="WEEKLY",amount=offering.offering_type==="ONE_TO_ONE"?offering.price_amount*count*(monthly?4:1):offering.price_amount;
  return (await pool.query(`INSERT INTO offering_orders(offering_id,cohort_id,learner_user_id,status,amount,currency,session_count,first_session_at,recurrence_frequency,booking_timezone,booking_settings,billing_type)
    ...
    [offering.id, cohortId || null, userId, status, amount, offering.currency,count,booking.sessionDates?.[0]||booking.firstSessionAt||null,booking.frequency||null,booking.timezone||null,JSON.stringify(booking),monthly?"MONTHLY":"ONE_TIME"])).rows[0];
}
...
async function fulfillOrder(orderId, paymentIntentId = null, free = false, stripeSubscriptionId = null) {
  ...
  try {
    ...
    const enrollment = (await client.query(`INSERT INTO journey_enrollments(learning_journey_id,learner_user_id,status,enrollment_source,enrolled_by_user_id,started_at)
      VALUES($1::uuid,$2::uuid,'ACTIVE','MANUAL',$3::uuid,NOW())
      ON CONFLICT(learning_journey_id,learner_user_id) DO UPDATE SET status='ACTIVE',enrollment_source='MANUAL',completed_at=NULL,updated_at=NOW()
      RETURNING *`, [order.learning_journey_id,order.learner_user_id,order.owner_user_id])).rows[0];
    ...
    if (order.offering_type==="ONE_TO_ONE") {
      ...
      if(order.billing_type==="MONTHLY"){const templates=[...dates];dates=[];for(let week=0;week<4;week++)for(const template of templates)dates.push(new Date(template.getTime()+week*7*86400000));}
